...
/**
 * Uploads photos to supabase storage and connects them to a catch
 * Returns a list of photos that failed to upload.
 */

export async function uploadCatchPhotos(localPhotos: string[], userId: string, catchId: number): Promise<string[]> {
  let failedPhotoUris: string[] = [];
  ...
  for (const [index, uri] of localPhotos.entries()) {
    try {
      ...
      const { data: publicUrl } = supabase.storage.from('catch_photos').getPublicUrl(filename);
